...
class BaseIBMAPIProcessor:
    ...
    def _reparse_response(self, response, system_content):
        ...
        user_prompt = rag_prompts.AnswerSchemaFixPrompt.user_prompt.format(
            system_prompt=system_content,
            response=response
        )

        reparsed_response = self.send_message(
            system_content=rag_prompts.AnswerSchemaFixPrompt.system_prompt,
...
class APIProcessor:
    ...
    def _build_rag_context_prompts(self, schema):
        """Return rag_prompts tuple for the given schema."""
        use_schema_prompt = True if self.provider == "ibm" or self.provider == "gemini" else False

        if schema == "name":
            system_prompt = (rag_prompts.AnswerWithRAGContextNamePrompt.system_prompt_with_schema
                             if use_schema_prompt else rag_prompts.AnswerWithRAGContextNamePrompt.system_prompt)
            response_format = rag_prompts.AnswerWithRAGContextNamePrompt.AnswerSchema
            user_prompt = rag_prompts.AnswerWithRAGContextNamePrompt.user_prompt
        elif schema == "number":
            system_prompt = (rag_prompts.AnswerWithRAGContextNumberPrompt.system_prompt_with_schema
                             if use_schema_prompt else rag_prompts.AnswerWithRAGContextNumberPrompt.system_prompt)
            response_format = rag_prompts.AnswerWithRAGContextNumberPrompt.AnswerSchema
            user_prompt = rag_prompts.AnswerWithRAGContextNumberPrompt.user_prompt
        elif schema == "boolean":
            system_prompt = (rag_prompts.AnswerWithRAGContextBooleanPrompt.system_prompt_with_schema
                             if use_schema_prompt else rag_prompts.AnswerWithRAGContextBooleanPrompt.system_prompt)
            response_format = rag_prompts.AnswerWithRAGContextBooleanPrompt.AnswerSchema
            user_prompt = rag_prompts.AnswerWithRAGContextBooleanPrompt.user_prompt
        elif schema == "names":
            system_prompt = (rag_prompts.AnswerWithRAGContextNamesPrompt.system_prompt_with_schema
                             if use_schema_prompt else rag_prompts.AnswerWithRAGContextNamesPrompt.system_prompt)
            response_format = rag_prompts.AnswerWithRAGContextNamesPrompt.AnswerSchema
            user_prompt = rag_prompts.AnswerWithRAGContextNamesPrompt.user_prompt
        elif schema == "comparative":
            system_prompt = (rag_prompts.ComparativeAnswerPrompt.system_prompt_with_schema
                             if use_schema_prompt else rag_prompts.ComparativeAnswerPrompt.system_prompt)
            response_format = rag_prompts.ComparativeAnswerPrompt.AnswerSchema
            user_prompt = rag_prompts.ComparativeAnswerPrompt.user_prompt
        elif schema == "string":
            # 新增：支持开放性文本问题
            system_prompt = (rag_prompts.AnswerWithRAGContextStringPrompt.system_prompt_with_schema
                             if use_schema_prompt else rag_prompts.AnswerWithRAGContextStringPrompt.system_prompt)
            response_format = rag_prompts.AnswerWithRAGContextStringPrompt.AnswerSchema
            user_prompt = rag_prompts.AnswerWithRAGContextStringPrompt.user_prompt
        else:
    ...
    def get_rephrased_questions(self, original_question: str, companies: List[str]) -> Dict[str, str]:
        ...
        answer_dict = self.processor.send_message(
            system_content=rag_prompts.RephrasedQuestionsPrompt.system_prompt,
            human_content=rag_prompts.RephrasedQuestionsPrompt.user_prompt.format(
                question=original_question,
                companies=", ".join([f'"{company}"' for company in companies])
            ),
            is_structured=True,
            response_format=rag_prompts.RephrasedQuestionsPrompt.RephrasedQuestions
        )
...
class BaseDashscopeProcessor:
    ...
    def send_message(
        ...
        if human_content:
            messages.append(Message(role='user', content=human_content))
