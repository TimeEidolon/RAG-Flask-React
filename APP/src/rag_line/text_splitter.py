...
class TextSplitter:
    ...
    def split_all_reports(self, reports, output_dir: Path, serialized_tables_dir: Optional[Path] = None):
        """
        ...
        for _report in reports:
            ...
            if serialized_tables_dir is not None:
                serialized_tables_path = serialized_tables_dir / _report.report_name
                if not serialized_tables_path.exists():
                    print(f"警告：未找到 {_report.report_name} 的序列化表格报告")

            # 读取报告内容
            with open(_report.merge_path, 'r', encoding='utf-8') as file:
            ...
            if temp:
                temp.chunked_path = file_name
                db.session.commit()

        # 输出处理文件数统计
        print(f"已分块处理 {len(reports)} 个文件")
