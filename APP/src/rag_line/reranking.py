...
from ..rag_line import rag_prompts
...
class JinaReranker:
    ...
    def rerank(self, query, documents, top_n: int = 10):
...
class LLMReranker:
    def __init__(self, provider: str = "dashscope"):
        ...
        self.system_prompt_rerank_single_block = rag_prompts.RerankingPrompt.system_prompt_rerank_single_block
        self.system_prompt_rerank_multiple_blocks = rag_prompts.RerankingPrompt.system_prompt_rerank_multiple_blocks
        self.schema_for_single_block = rag_prompts.RetrievalRankingSingleBlock
        self.schema_for_multiple_blocks = rag_prompts.RetrievalRankingMultipleBlocks
    ...
    def get_rank_for_single_block(self, query, retrieved_document):
        ...
        if self.provider == "openai":
            ...
            messages = [
                Message(role="system", content=self.system_prompt_rerank_single_block),
    ...
    def get_rank_for_multiple_blocks(self, query, retrieved_documents):
        ...
        if self.provider == "openai":
            ...
            messages = [
                Message(role="system", content=self.system_prompt_rerank_multiple_blocks),
                Message(role="user", content=user_prompt),
            ]
            rsp = self.llm.Generation.call(
                model="qwen-turbo",
